    ...
    )
{
    ...
    snprintf(cmd, sizeof(cmd), "%s %s | grep -i speed", ETHSWITCHTOOL, ifName);
    fp = popen(cmd, "r");
    if(fp != NULL)
    {
    ...
    if(speed)
    {
        ...
        snprintf(cmd, sizeof(cmd), "%s %s | grep -i duplex", ETHSWITCHTOOL, ifName);
        fp = popen(cmd, "r");
        if(fp != NULL)
        {
            while (fgets(buf, sizeof(buf), fp) != NULL) {
	...
	else
	{
		for(count = 0;count < maccount ; count++)
		{
			if (fgets(str, MAX_BUF_SIZE, fp) == NULL)
				break;
			/* copy safely; str[17] = XX:XX:XX:XX:XX:XX + NUL */
			{
